(function (factory) {
	...
	L.Control.Compass = L.Control.extend({
		...
		onAdd: function (map) {
			...
			L.DomEvent.on(this._button, "click", L.DomEvent.stop, this).on(
				this._button,
				"click",
				this._switchCompass,
				this
			...
			if (this.options.autoActive) this.activate(true);
		...
		onRemove: function (map) {
			...
			L.DomEvent.off(this._button, "click", L.DomEvent.stop, this).off(
				this._button,
				"click",
				this._switchCompass,
				this
		...
		_rotateHandler: function (e) {
			...
			if (angle % this.options.angleOffset === 0) self.setAngle(angle);
		},
		...
		activate: function (isAutoActivation) {
			if (
				...
				DeviceOrientationEvent.requestPermission().then(
					function (permission) {
						if (permission === "granted") that._activate();
